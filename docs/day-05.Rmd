...
```{r}
covid %>%
  mutate(state_deaths_to_cases = deaths/cases) %>%
  filter(cases != 0) %>%
  group_by(state) %>%
  summarize(state_deaths_to_cases = sum(state_deaths_to_cases, na.rm = TRUE)) %>%
  arrange(desc(state_deaths_to_cases)) %>%
  top_n(5)
